...
- 2026-09-08: A creating `POST` answers `201 Created` with a `Location` header naming the new resource: contact groups, message tags, saved searches, API tokens, owner-created accounts, imports and asset uploads. (#498)
- 2026-09-08: Every response carries an `x-request-id` the server made, and every failure repeats it in the body, so a person quoting a failure and the operator reading the log are looking at the same request. (#499)
...
- 2026-09-05: The release Docker image compiles the vault with the Rust that `rust-toolchain.toml` pins, not with whatever the `rust:` base tag carried (1.95 while the pin said 1.98.1), so the published server is built on the compiler CI tested. `scripts/check-docker-context.sh` fails when the Dockerfile stops copying the pin or its base image moves to another minor. (#427)
...
- 2026-09-08: `/v1/accounts` is the one collection for accounts, for the vault owner and for each account alike. `GET /v1/accounts` lists them for the owner; `POST /v1/accounts` creates one, as the owner with a first password or as a stranger while the vault is open, and answers `201 Created` with `Location: /v1/accounts/{id}`; `GET`, `PATCH` and `DELETE /v1/accounts/{id}`, `PUT /v1/accounts/{id}/password`, `DELETE /v1/accounts/{id}/messages`, `GET /v1/accounts/{id}/storage` and `/v1/accounts/{id}/api-tokens` are reached by the owner or by the account itself, decided in the handler rather than by the path. `/v1/account/*`, `/v1/owner/accounts*` and `POST /v1/auth/register` are gone. (#511)
- 2026-09-08: Accounts and API tokens are addressed by integer ids like every other row: `account_id` on every response, `/v1/owner/accounts/{id}` and `/v1/account/api-tokens/{id}` take an integer, and the vault owner is always account 1 and the demo account 2. The UUID strings are gone, and so is the stub account the server CLI created for an unknown `--account` UUID; `--account` names an existing username or id. Vault schema 14; existing databases are rebuilt empty. (#511)
