...
- **`crates/vault/server`** — each `*_api.rs` file is one Axum route group; `db/` modules mirror the table sources in the repo-root `schema/sql/*.sql`, which the server embeds at compile time (`db/schema.rs`) — change tables there, not in a live db file. Import path: `jsonl.rs` reads the records, then `imports_api/` runs `staging` → `promote` → `contact_name`, calling `dedupe.rs`; demo mode is a seed action, not a runtime mode — `reset-demo` (`reset_demo.rs`) writes two account rows: `demo` at the fixed `DEMO_ACCOUNT_ID` with a NULL password hash, which is why `demo` logs in with an empty password, and the owner `admin` (password `admin`) at `OWNER_ACCOUNT_ID`. Nothing at request time knows a vault is a demo; the only request-time check is `is_demo_account`, which reports `is_demo` on the profile and refuses to delete the demo account. A test module over a few hundred lines lives beside its source as `<module>/tests.rs` (declared `mod tests;`), so the source file stays readable; small ones stay inline.
...
- **Starting the vault, the browser UI, or the desktop app** — "Run the vault (development)": dev script flags, the demo login, the Postgres variant, and what must not run at the same time.
