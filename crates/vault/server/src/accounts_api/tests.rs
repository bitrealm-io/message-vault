...
/// Registration counts once for the whole vault, like claiming: a count per
/// username would let a script that tries a new name each time straight
/// through.
#[tokio::test]
async fn registrations_under_many_names_hit_the_rate_limit() {
    let vault = test_vault().await;
    let state = vault.state.clone();
    let server = crate::test_support::serve(&state).await;
    let register = async |username: String| {
        // No password, so no Argon2 hash slows the loop down.
        reqwest::Client::new()
    ...
    for n in 0..crate::credentials::AUTH_RATE_MAX {
        let response = register(format!("stranger{n}")).await;
        assert_eq!(response.status(), StatusCode::CREATED, "registration {n}");
    }
    let response = register("one-more-name".to_string()).await;
