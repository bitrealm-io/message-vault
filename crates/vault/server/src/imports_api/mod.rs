...
pub(crate) struct CreateImportRequest {
    ...
    /// Absolute staging path on the client that owns this run.
    #[serde(default)]
    pub(crate) staging_dir: Option<String>,
    /// Which install is creating the run.
    #[serde(default)]
...
/// Stored run status after completion.
#[derive(Debug, Serialize, utoipa::ToSchema)]
...
/// Confirmation that a run was discarded.
#[derive(Debug, Serialize, utoipa::ToSchema)]
