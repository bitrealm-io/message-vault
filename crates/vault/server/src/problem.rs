...
`errors` lists every rule the request broke, one sentence each, not only the first. Fix each one and send the request again.".to_string(),
            ...
            Self::InvalidCredentials => "The username or password did not match an account, or the current password given to confirm deleting an account was wrong. The vault does not say which half failed. Check both and try again; repeated attempts are rate limited.".to_string(),
