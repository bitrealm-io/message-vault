...
pub enum ApiError {
    ...
    NotTheOwner(String),
    /// `403` — a stranger registered while the vault does not accept new accounts.
    ...
    /// `422` — the body does not hash to its address, or a part, upload id or completion does not match the upload.
...
pub(crate) fn http_app(state: AppState) -> Router {
    ...
    spec.merge(rest);
    let declared = crate::extract::DeclaredQueryParameters::from_openapi(&spec);
    ...
    let mut api = Router::new()
        ...
        // Also per route: the matched route's template says which query
        // parameters it declares.
        .route_layer(axum::middleware::from_fn_with_state(
            declared,
            crate::extract::refuse_undeclared_query_parameters,
        ))
