...
- `GET /v1/accounts/{id}/storage` is the attachment total, the attachment, conversation and contact counts, and the largest files. `GET /v1/vault/storage` is the same counts and total summed over every account, for the owner only, plus what the database takes on disk: `database_bytes`, `messages_bytes` (the messages table and its indexes, without full-text search) and `fts_bytes` (the search index, for the whole vault), all measured, and an `accounts` list with each account's message count, text bytes and `estimated_message_bytes`, which is `messages_bytes` split by each account's share of text. How each figure is measured on each engine is on [Database](/vault/developer/reference/database/).
