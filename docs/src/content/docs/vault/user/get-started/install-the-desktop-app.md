...
Log in with **http://127.0.0.1:8080** and the vault username and password, then [Import from a backup](/vault/user/import-from-a-backup/). The desktop app uses the IPv4 address because `localhost` can resolve to IPv6, which a local Docker vault does not listen on.
