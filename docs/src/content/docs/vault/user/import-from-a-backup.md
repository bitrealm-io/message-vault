...
2. **Staging Approval.** The run stops and shows what it needs you to weigh, read from the staged files rather than estimated: the contacts in the backup, split into **Existing** and **New** to your vault; the **Size limit per file** and the **Files over the limit**, which opens to each file and its size; and the identities the backup's outgoing messages were sent from, each marked as on your profile or not. When you chose Convert or Compress, a **Conversion estimates** or **Compression estimates** group sorts the large files into **Likely within limit**, **May exceed limit** and **Not audio or video**, and each opens to its files with the staged size and the expected one. These are estimates, because Media has not run yet. The button continues the run, **Upload to vault**, **Convert media** or **Compress media**; **Cancel this import** ends the run and deletes what was staged.
...
4. **Media Approval.** Media's row shows the new total size and how many files could not be converted or compressed. The approval shows what is true now: the size limit per file and the files still over it, by name and size. A file over the limit stays out of the vault; its message keeps its text and a placeholder. **Upload to vault** continues, or cancel.
